...
export default function CountboardDashboardAssembly() {
  ...
  const {
    ...
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/machines?type=assembly`,
    fetcher,
    {
  ...
  const hourlyDataKey = selectedMachine?.machineName
    ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/machines/hourly/${selectedMachine.machineName}?type=assembly${
        !isLiveMode &&
        new URLSearchParams(window.location.search).get('date') !== null
          ? `&date=${new URLSearchParams(window.location.search).get('date')}&shift=${new URLSearchParams(window.location.search).get('shift')}`
          : ''
  ...
  const handleScrapUpdate = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/countboards/scrap-assy`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const handleReworkUpdate = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/countboards/rework-assy`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const totalActualEK =
    ...
    0
  console.log(`totalActualEK : ${totalActualEK}`)
  ...
  return (
    <div className="p-0 space-y-2 w-full">
      ...
      {selectedMachine === null && isLoading == false ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-2 text-center w-full mb-2">
          ...
          <Card className="p-0">
            ...
            <CardContent className="grid grid-cols-3 gap-6 p-2 pr-4">
              ...
              <div>
                <div className={`text-5xl font-bold`}>{totalActual}</div>
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-3 gap-6 p-2 pr-4">
              <div>
                <div className={`text-5xl font-bold`}>{totalRejectOverall}</div>
              ...
              <div>
                <div
                  className={`text-5xl font-bold ${totalRejectOverall / totalActual > rejectPercentage ? 'text-red-500' : 'text-green-500'}`}
                >
          ...
          <Card>
            <CardHeader className="p-2 pb-1">
              {/* Gunakan grid-cols-2 agar pembagian kolomnya sama persis dengan Content */}
              <div className="grid grid-cols-2 gap-4 text-center">
                <div className="flex justify-center">
                  <div className="text-lg font-bold text-red-500 uppercase bg-red-200 px-2 rounded">
                    Non O.O.E
                  </div>
                </div>
                <div className="flex justify-center">
                  {/* Badge status OK/NG disesuaikan kondisinya jika perlu */}
                  <div className="text-lg font-bold text-green-500 uppercase bg-green-200 px-2 rounded">
                    OK
                  </div>
                </div>
              </div>
            ...
            <CardContent className="grid grid-cols-2 gap-4 p-2 pt-0 text-center">
              <div>
              ...
              <CardContent>
                <div className="w-full flex overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className="w-[80px] text-lg text-nowrap font-bold">
                          Time
                        </TableHead>
                        <TableHead className="w-[150px] text-lg text-nowrap font-bold">
                          ItemNo
                        </TableHead>
                        <TableHead className="w-[80px] text-center text-lg text-nowrap font-bold">
                          Target
                        </TableHead>
                        <TableHead className="w-[250px] text-center text-lg text-nowrap font-bold">
                          Actual Qty
                        </TableHead>
                        <TableHead className="w-[50px] text-center text-lg text-nowrap font-bold">
                          Gap
                        </TableHead>
                        ...
                        <TableHead className="w-auto min-w-[200px] text-center text-lg font-bold">
                          Causes
                        </TableHead>
                        <TableHead className="w-auto min-w-[200px] text-center text-lg font-bold">
                          Comments/Actions
                        ...
                        hourlyData?.map((row, index) => {
                          ...
                          return (
                            <TableRow className="h-12" key={row.time}>
                              ...
                              <TableCell
                                className={`text-center h-full text-nowrap border-l ${textAnimation}`}
                              >
                                {target_show}
                              </TableCell>
                              ...
                              <TableCell
                                className={`w-[50px] text-center font-semibold ${displayDelta >= 0 ? 'text-green-600' : 'text-red-600'}`}
                              >
                                {displayDelta}
                              </TableCell>
                              <TableCell
                                className="h-[50px] bborder-x border-gray-300 text-center text-xl text-black cursor-pointer"
                                onClick={() => {
                              ...
                              <TableCell
                                className="w-[50px] border-r border-gray-300 text-center text-xl text-black cursor-pointer"
                                onClick={() => {
                                  setSelectedHourlyId(row.hourlyId)
                              ...
                              <TableCell
                                ...
                                className="text-left px-4 cursor-pointer hover:bg-gray-50 min-w-[200px]"
                              >
                                <Tooltip>
                                  <TooltipTrigger asChild>
                            ...
                            (Click to edit causes)`
                              ...
                              <TableCell
                                ...
                                className="text-left px-4 cursor-pointer hover:bg-gray-50 min-w-[125px]"
                              >
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <div className="line-clamp-2 text-sm text-center">
