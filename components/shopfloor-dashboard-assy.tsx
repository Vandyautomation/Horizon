...
function InjectionMoldingMachine({
  ...
  const { scene } = useGLTF('/admin/assets/3d/assy_color.glb');
  const clonedScene = useMemo(() => scene?.clone(), [scene]);
    ...
    <group position={machine.position} onClick={onClick}>
      ...
      <Html position={[0, 3, 0]} center>
        <div
          ...
          className={`bg-opacity-50 ${machine.status === 'WHITE' ? 'text-black' : 'text-white'} p-1 rounded text-base text-nowrap${
            isSelected ? 'font-bold' : ''
          }`}
        >
          {machine.id}
...
export default function ShopfloorDashboard() {
  ...
  const key = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/andon/buildings/ASSEMBLY`;
  ...
  let queryLocation = searchParams.get('building') || '';
    if (queryLocation == '') {
    queryLocation = 'ASSY Bld J';
    params.set('building', 'ASSY Bld J');
  }
    ...
    <div className="w-full h-[875px] p-0 m-0">
      <div className="grid grid-cols-3 gap-2 m-0">
        ...
        <Card className="absolute top-[70px] left-1/2 -translate-x-1/2 z-10">
          <CardHeader>
            <CardTitle className="text-3xl">ASSEMBLY BUILDING {selectedBuilding?.name.split(' ')[2]}</CardTitle>
          </CardHeader>
        </Card>
      </div>
      ...
      {Array.isArray(selectedBuilding?.machines) && selectedMachine && (
        <Card className="absolute bottom-20 left-4 w-[350px] z-20">
          <CardHeader>
