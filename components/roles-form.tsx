...
export function RolesForm() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/qco/api/roles`,
        {
          // credentials: 'include',
        }
      )
  ...
  const updateRole = async (updatedRole: Roles) => {
    await toast.promise(
      fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/qco/api/roles/${updatedRole.id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Enter the details of the new roles here. Click save when
                  you&apos;re done.
                </DialogDescription>
              </DialogHeader>
              <form
