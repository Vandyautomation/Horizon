...
export default function EmsDashboardOverall() {
    ...
    <div className="p-0 space-y-2 w-full  overflow-x-hidden">
      <div className="flex flex-wrap gap-2 pt-0">
        ...
        >
          <RefreshCw
            className="w-4 h-4"
            style={{
              animation: isLoadingRefresh ? 'spin 2s linear infinite' : 'none',
            }}
          />
        </Button>
      ...
      <div className="w-full grid grid-cols-1 gap-2 mb-0 pb-0">
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleToleranceChange(value)}
                >
        ...
        {selectedMachine === null || !Array.isArray(processedNoeeData) ? (
          <div className="text-center">
            Please select machine (or wait a moment)...
          </div>
        ) : (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="h-2 p-0">
                          ...
                          {[...Array(24)].map((_, hour) => (
                            <TableCell className="w-10 h-2 " key={hour}>
                              {`${hour < 10 ? '0' : ''}${hour}`}:00
                            </TableCell>
                          ))}
                        </TableRow>
                      ...
                      <TableBody>
                        {fiveMinutes.map((nooe, rowIndex) => (
                          <TableRow key={rowIndex} className="h-2 p-0">
                            <TableCell className="h-2 text-xs w-8 text-center p-0">
                              {nooe.time}
                            </TableCell>
                            {[...Array(24)].map((_, hour) => {
                              const date = new Date();
                              ...
                              return (
                                <TableCell
                                  key={hour}
                                  className="w-8 h-2 p-0 pl-2  text-center items-center justify-center"
                                >
                                  <div
                                    className={`w-10 h-2 items-center justify-center p-0 m-0 ${
                                      activeColor
                                        ? colorMap[
                                            activeColor as keyof typeof colorMap
                                          ]
                                        : 'bg-transparent'
                                    }`}
                                  />
