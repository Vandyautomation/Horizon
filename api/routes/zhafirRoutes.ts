...
zhafirRoutes.get('/', async (c) => {
  try {
    ...
    const section = c.req.query('section')
    const machineId =
      c.req.query('machine_id') || c.req.query('machineId') || undefined
...
zhafirRoutes.get('/actual-view', async (c) => {
  try {
    ...
    if (date && hourRaw !== undefined) {
      ...
      const data = await getZhafirActualFromViewByHour(
        paraId,
        machineId,
        date,
        hour
...
zhafirRoutes.post('/material-type-routing', async (c) => {
  try {
    ...
    const materialId = (body.material_id || body.materialId) as
      | string
      | undefined
    const materialType = (body.materialType || body.type || body.material) as
      | string
      | undefined
    ...
    const routingUpdate = await updateRoutingMaterialTypeByMaterialId(
      materialId,
      materialType
    )
    const trxUpdate = await updateLatestTrxMaterialByMachine(
      machineId,
      materialType
    )
...
zhafirRoutes.post('/manual-actual', async (c) => {
  try {
    ...
    const data = await updateHardcodedActField(
      field,
      valueRaw as any,
      machineId
    )
